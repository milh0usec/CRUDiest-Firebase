app.controller('HomeController', ['$scope', '$http', '$firebaseArray', '$firebaseAuth', '$uibModal', function($scope, $http, $firebaseArray, $firebaseAuth, $uibModal) {
  ...
  // Set up Firebase2 auth
  // $scope.authObj = $firebaseAuth(ref);
  // Set up Firebase3 auth
...
$scope.loginAnon = function() {
  $scope.authData = null;
  $scope.error = null;
  // $scope.authObj.$authAnonymously().then(function(authData) { // AngularFire 1
  $scope.authObj.$signInAnonymously().then(function(authData) { // AngularFire 2
    $scope.authData = authData;
...
$scope.loginGoogle = function() {
  $scope.authData = null;
  $scope.error = null;
  $scope.authObj.$signInWithPopup("google").then(function(authData) {  // AngularFire 2
    $scope.authData = authData;
    $scope.authData.uid = authData.user.providerData[0].uid; // fixes bug from migration from Firebase 2 to Firebase 3
...
$scope.loginFacebook = function() {
  $scope.authData = null;
  $scope.error = null;
  $scope.authObj.$signInWithPopup("facebook").then(function(authData) {  // AngularFire 2
    $scope.authData = authData;
    $scope.authData.uid = authData.user.providerData[0].uid; // fixes bug from migration from Firebase 2 to Firebase 3
    console.log("Logged in as:", authData.uid);
  }).catch(function(error) {
...
$scope.loginTwitter = function() {
  $scope.authData = null;
  $scope.error = null;
  $scope.authObj.$signInWithPopup("twitter").then(function(authData) {  // AngularFire 2
    $scope.authData = authData;
    $scope.authData.uid = authData.user.providerData[0].uid; // fixes bug from migration from Firebase 2 to Firebase 3
    console.log("Logged in as:", authData.uid);
  }).catch(function(error) {
...
$scope.loginGitHub = function() {
  $scope.authData = null;
  $scope.error = null;
  $scope.authObj.$signInWithPopup("github").then(function(authData) {  // AngularFire 2
    $scope.authData = authData;
    $scope.authData.uid = authData.user.providerData[0].uid; // fixes bug from migration from Firebase 2 to Firebase 3
    console.log("Logged in as:", authData.user.providerData[0].displayName);
  }).catch(function(error) {
    console.error("Authentication failed:", error);
  });
};
...
$scope.logout = function() {
  ...
  // $scope.authObj.$unAuth(); // AngularFire 1
  $scope.authObj.$signOut(); // AngularFire 2
  // $scope.authObj.$onAuth(function(authData) { // AngularFire 1
  $scope.authObj.$onAuthStateChanged(function(authData) { // AngularFire 2
    if (authData) {
      console.log("Logged in as:", authData.uid);
    } else {
      $scope.authData = null;
